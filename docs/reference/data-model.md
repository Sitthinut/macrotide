...
| `fund_catalog` | SEC-sourced fund universe (parent-level: one row per `proj_id`) | keyed by `proj_id`; `current_ter` is a **derived cache** of the latest TER (maintained by `upsertFundFees`; source of truth stays `fund_fees`) — picked from the **representative retail class**, not a fee-waived sibling, so the parent fee reflects what a retail buyer pays; `proj_retail_type` (`R` = retail, else not-for-retail) is the screener's fund-level retail gate; `asset_class` is derived from the SEC `policy_desc` label, except **money market** (which `policy_desc` lumps under bond) is recovered from the fund name → `cash` |
