...
| `/api/import/broker/userscript/[token]/[name]` | GET | Serves the install-ready userscript for the configured brokers — the full loader for a `.user.js` `[name]`, or the metadata-only block for a `.meta.js` (the manager's `@updateURL` version check). Authenticated by the import `[token]` in the path, NOT a session cookie, so the manager's cookie-less install/update fetch succeeds (CORS-opened for the same reason); the served script carries `@downloadURL`/`@updateURL` for manager auto-update on a protocol bump. Broker endpoints stay server-side (env only); 404 when no broker is configured; 401 for an unknown/rotated token |
