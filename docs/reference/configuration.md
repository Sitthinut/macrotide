...
| `PUBLIC_TIER_MODEL` | `openrouter/free` | [lib/ai/provider.ts](../../lib/ai/provider.ts) | Model chain for `tier='public'` chat. Read from its OWN var, never from `AI_MODELS` (the invariant below). Defaults to the zero-cost free router; point it at a cheap PAID model (e.g. `google/gemini-2.5-flash`) to lift public-tier quality — bounded by the daily token + optional cents caps. (`FREE_TIER_MODEL` is a deprecated-but-honored alias.) |
| `REASONING_GATE` | `on` | [lib/advisor/intent.ts](../../lib/advisor/intent.ts), [app/api/chat/route.ts](../../app/api/chat/route.ts) | Owner/trusted only: a deterministic classifier raises reasoning `effort` to `medium` on genuine multi-step turns (rebalance, SSF-vs-RMF, plan-anchored tilt) and `none` otherwise. Public/demo always stay `none` (cost-protected). Set `off` to inherit each model's default reasoning instead. Tune the trigger set against the eval (`EVAL_TIER=complex EVAL_REASONING=medium`). |
...
The public-tier **model chain** is derived ONLY from its own `PUBLIC_TIER_MODEL` var
...
conscious operator act (`PUBLIC_TIER_MODEL=…`), and public spend stays bounded by the
...
| Var | Default | Read by | Notes |
| --- | --- | --- | --- |
| `DAILY_TOKEN_BUDGET_PUBLIC` | `20000` | [lib/db/queries/usage.ts](../../lib/db/queries/usage.ts) | Daily input+output token cap per `tier='public'` user. Checked before forwarding to OpenRouter; resets at UTC midnight. Malformed/≤0 → default. Always on (the floor). (`DAILY_TOKEN_BUDGET_FREE` is a deprecated-but-honored alias.) |
| `DAILY_TOKEN_BUDGET_TRUSTED` | `200000` | [lib/db/queries/usage.ts](../../lib/db/queries/usage.ts) | Same, for `tier='trusted'` users. |
| `DAILY_CENTS_BUDGET_PUBLIC` | unset → **cost cap OFF** | [lib/db/queries/usage.ts](../../lib/db/queries/usage.ts) | Optional daily **cost** ceiling in US cents per `tier='public'` user — the right bound when `PUBLIC_TIER_MODEL` is a paid model with asymmetric in/out pricing. Checked alongside the token cap (either tripping blocks the turn). Unset or malformed/≤0 → disabled (no invented money cap; the token cap still applies). (`DAILY_CENTS_BUDGET_FREE` is a deprecated-but-honored alias.) |
